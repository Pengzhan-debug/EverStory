...
7. **多智能体协作但不越权**：角色拥有独立名称、头像、职责和 API 路由；群聊结论
   默认只是“假设”，智能体之间可以点名质疑。
8. **玩家审批 + 证据板**：智能体可提出前往、询问、检查和指控等结构化行动；审批前
   世界不变，审批后由状态机校验并执行，证据记录带地点、来源、任务和确认回合。
9. **模型控制台**：在 `/settings` 配置多个兼容 API、按角色分配连接，并查看延迟、
   Token、失败率和调用状态。
10. **完整破案闭环**：灯塔案件包含三名嫌疑人、互相矛盾的证词、可检查物证和最终
    指控；缺少任一关键证据时指控不会结案，证据链完整后才触发确定性招供结局。
...
多智能体调查基准在离线 CI 中取得：结构化提案准确率 **100%（6/6）**、批准执行
成功率 **100%（6/6）**、越权状态写入 **0 次**、证据落板率 **100%（6/6）**，
同时验证过期任务拦截、调查记忆存档恢复和最终结案。详见
[`docs/eval-report-team.md`](docs/eval-report-team.md)。API 模式还会记录各智能体的调用数、
延迟和 Token。
...
测试套件包含一条无需 API Key 的完整离线验收路径：智能体提议移动、两次证人询问、
两件物证检查以及受证据链约束的最终指控。
